...
export function insertPresetText() {
...
export function copycode(obj) {
    copyToClipboard($(obj.currentTarget).closest('code').clone().children('button').remove().end().text());
    layer.msg("复制完成！");
}
...
export function popupPanel(content, size) {
    layer.open({
        type: 1,
        title: false,
        closeBtn: 0,
